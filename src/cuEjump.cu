...
extern __constant__ const float d_Fcoul_scale;  //! íå ïîìíþ òî÷íî
...
__global__ void cuda_ejump(int bndPerThread, cudaMD* md)
{
    ...
    while ((i < N) && (cultBnd < nbonds))   // end cycle then all bonds became cultivated (or end of interval)
    {
        ...
        //! òóò áû íàäî ó÷åñòü âîçìîæíîñòè îáðàçîâàíèÿ âîäîðîäíûõ ñâÿçåé, òîãäà nbonds ìîæåò íå ñîîòâåòñòâîâàòü ôàêòè÷åñêîìó êîë-âó
        //! íî äëÿ ïðîñòîòû ìû ïîëîæèì, ÷òî àòîìû äîíîðîâ/àêöåïòîðîâ íå ìîãóò ó÷àâñòâîâàòü â âîäîðîäíûõ ñâÿçÿõ
        //! õîòÿ âîçìîæíî, åñëè ìû áóäåì çàïèñûâàòü âîäîðîäíûå ñâÿçè â nbonds îáîèõ àòîìîâ, êàê îáû÷íûå ñâÿçè ýòî íå íàðóøèò îáùíîñòè? ( ÿ âèäèìî ñäåëàë, ÷òîáû âàëåíòíûå óãëû íå ïîðòèëèñü)
        //! íàâåðíîå, ýòî ìîæíî ïîôèêñèòü, åñëè ñäåëàòü óãîë çàâèñèìûé îò ëèãàíäà
        ...
        if (nei > -1)
        {
            ...
            if (vdw != NULL)
                dU -= vdw->eng_r(r2, r, vdw);   //! íóæíî ââåñòè ôóíêöèþ, êîãäà r òî÷íî èçâåñòíî, áåç îäíîé ïðîâåðêè r
    ...
    if (threadIdx.x == 0)
    {
        ...
        if (do_jmp)
        {
            ...
            //! íå çíàþ ïîêà çà÷åì, â îòëè÷èå îò ñåðèéíîé âåðñèè òóò âñå ðàâíî íå ñäåëàåøü íåñêîëüêî ýëåêòðîííûõ øàãîâ çà îäèí îáû÷íûé.... íî íà áóäóùåå
