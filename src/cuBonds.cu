...
__device__ void try_to_bind(float r2, int id1, int id2, int spec1, int spec2, cudaMD *md)
{
    ...
    // íàäî áû èñêëþ÷èòü ñàìîñâÿçûâàíèå, õîòÿ áû ïîïûòàòüñÿ
    ...
    if (md->bindBonds[spec1][spec2] != 0)
    {
        if (r2 < md->bindR2[spec1][spec2])
        {
            ...
            {
                // òóò åñòü îïàñíîñòü, ÷òî ïîêà ìû ïðèñòóïèëè ê ýòîìó, êòî-òî óæå ïåðåçàïèñàë ìèíèìóì, íó ïðèíåáðåæåì ýòèì, êòî óñïåë òîò è óñïåë
                //atomicExch(&(md->neighToBind[id1]), id2);   
                
                // !ìîæíî íàâåðíî è áåç àòîìèê:
...
/*
    if (clear_1 || clear_2)
    {
        for (i = 0; i < md->nBond; i++)
            if (md->bonds[i].z != 0)    // ñþäà æå âõîäèò è ñâÿçü id1-id2, ïîñêîëüêó îíà ïðåäâàðèòåëüíî áûëà îáíóëåíà â apply_bonds
            {
                if (md->bonds[i].x == id1)
                {
                    if (clear_1)
        ...
        if (clear_1)    // åñëè äàæå ïîñëå ýòîãî íå î÷èñòèëèñü - çàíóëÿåì
            md->parents[id1] = -1;
        if (clear_2)    
            md->parents[id2] = -1;
    }
...
////////////  end H-bonds addition
    ...
    //! íà âñÿêèé ñëó÷àé (íà ñàìîì äåëå, åñëè òàêîå ñëó÷èëîñü, ÷òî-òî ïîøëî íå òàê, çàâåðíóòü â DEBUG)
    if (clear_1)    // åñëè äàæå ïîñëå ýòîãî íå î÷èñòèëèñü - çàíóëÿåì
        md->parents[id1] = -1;
    if (clear_2)
        md->parents[id2] = -1;
...
//  exclude pid from parent of H-atom (atoms[hid]), seek another parent for hid if acceptable
{
    ...
    if (md->parents[hid] == pid)
    {
        if (md->nbonds[hid] > 0)    // there are bonds with this H-atom, seek a new parent
        {
            ...
            //! íà âñÿêèé ñëó÷àé (âäðóã ÷òî-òî ïîøëî íå òàê, çàâåðíóòü â äåáàã)
            if (md->parents[hid] == pid)
            {
                md->parents[hid] = -1;      // åñëè äàæå ïîñëå ýòîãî íå î÷èñòèëèñü - çàíóëÿåì
            }
        }
        else // if there are no bonds with the atom, just reset its parent
            md->parents[hid] = -1;
...
/////     end H-bonds addition
          ...
          while (loop)
          {
             if (save_lt)       // ýòîò ôëàã êðîìå âñåãî ïðî÷åãî, ñèìâîëèçèðóåò, ÷òî âíà÷àëå íàì óæå ïîòðåáîâàëîñü ïåðåîïðåäåëèòü ñâÿçü, ïðè ïîâòîðíîì ïðîõîæäåíèè öèêëà, îí âñåãäà áóäåò true
             {
                 ...
                 {
                     //! ìåíÿåì ñâÿçü è ïîåõàëè äàëüøå
                     if (def < 0)  // åñëè îáðàòíàÿ, ìåíÿåì îïÿòü æå ïîðÿäîê àòîìîâ â ñâÿçè
                     {
                         invert_bond(id1, id2, spec1, spec2, &(md->bonds[iBnd]));
                         def = -def;
                     }
             ...
             if (action)
             {
                 ...
                 {
                     spec1 = old;
                     spec2 = md->types[id2];   // îáíîâëÿþ çíà÷åíèå 2ãî òèïà àòîìà, âäðóã îíî òîæå èçìåíèëîñü
                 ...
                 else      // types[id1] have been changed
                 {
                     ...
                     {
                         //! íàèõóäøèé âàðèàíò: À óñïåëè ïîìåíÿòü, à Á - íåò
                         // ïðåäñòàâëÿåì, ÷òî ó íàñ áûëà ñâÿçü A-old è îíà ðåàãèðóåò òàêæå íà òàêóþ äëèíó ñâÿçè
                     ...
                     // ñîõðàíÿåì, ÷òî ìû èçìåíÿëè ýòîò òèï àòîìà, åñëè ýòî íå áûëî ñîõðàíåíî ðàíåå
                     //if (md->use_angl == 2) // variable angles
          ...
          // åäèíñòâåííûé ñëó÷àé, êîãäà íóæíî îáíîâèòü bnd
          if (action == 2)
          {
#ifdef DEBUG_MODE
...
////////////  end H-bonds addition
              ...
              //atomicAdd(&(cur_bnd->count), 1);    // òåïåðü â if (save_lt)
          }
          ...
          if (action != 1)  // not delete
          {
              ...
              //! ÂÐÅÌÅÍÍÎ! äëÿ óðàâíîâåøèâàíèÿ ñâÿçåé
              if ((f > -0.1f) && (f < 0.1f))
              {
...
////////////  end H-bonds addition
                  atomicSub(&(md->nbonds[id2]), 1);
              md->bonds[iBnd].z = 0;
              //atomicSub(&(old_bnd->count), 1);    // òåïåðü â if (save_lt)

              //! çäåñü è âåçäå íàäî ñäåëàòü ïåðåñ÷åò êîëè÷åñòâ ñïåöèåâ
...
//    int spec1, spec2;     // atom types
    ...
    for (iBnd = id0; iBnd < N; iBnd++)
            if (md->bonds[iBnd].z)  //! âîîáùå çäåñü ýòó ïðîâåðêó ìîæíî è îïóñòèòü
            {
                // atom indexes
                id1 = md->bonds[iBnd].x;
                id2 = md->bonds[iBnd].y;
...
// connect atoms which are selected to form bonds
{
    ...
    for (iat = id0; iat < N; iat++)
    {
        ...
        if (nei)    // neighbour exists
        {
            ...
            //! òóò ñðàçó ïðîâåðèì è íà parents (èñêëþ÷àåì äâîéíîå ñâÿçûâàíèå):
            //! âîîáùå ýòà ïðîâåðêà óæå âûïîëíÿåòñÿ â try to bind, òàê ÷òî òóò ìîæåò è ñêàíàåò áåç íåå
            ...
            //! òóò òàêàÿ øòóêà, åñëè îñòàâèòü êàê áûëî, ïåðâûì îáðàòûâàåòñÿ àòîì iat, à âòîðûì nei
            //!  òî ó íàñ ðàçíûå ïîòîêè áåðóåòñÿ çà ýòî äåëî ñ ðàçíûõ ñòîðîí è ïîëó÷àåòñÿ îäèí ëî÷èò ïåðâûé àòîì â ïàðå, äðóãîé - âòîðîé
            //!  è êîãäà îíè äîáèðàþòñÿ äî 2îé ïðîâåðêè ïîëó÷àþò ôîëñ è âûõîäÿò, ïîýòîìó ñðàçó óïîðÿäî÷èì èíäåêñû
            if (iat < nei)
            {
                id1 = iat;
                id2 = nei;
            ...
            {
                //continue;   // ïîïðîáóåì ïðîñòî âûéòè
                id1 = nei;
                id2 = iat;
            }
            ...
            // ñðàçó ïðîâåðèì, ìîãóò ëè ýòè àòîìû îáðàçîâàòü ñâÿçü, åñëè íåò - âûõîäèì
            if (btype == 0)
            {
...
#endif
            // ñîõðàíÿåì, ÷òî ìû èçìåíÿëè ýòè òèï àòîìà, åñëè ýòî íå áûëî ñîõðàíåíî ðàíåå
            //if (md->use_angl == 2)      // variable angles
            //{
                atomicCAS(&(md->oldTypes[id1]), -1, spec1);
                atomicCAS(&(md->oldTypes[id2]), -1, spec2);
            ...
            */
            ...
            // ïîïðîáóåì çàíóëèòü ñêîðîñòè ÷àñòèö, îáðàçóþùèõ ñâÿçü
            //md->vls[id1] = make_float3(0.f, 0.f, 0.f);
            //md->vls[id2] = make_float3(0.f, 0.f, 0.f);
            ...
            // ïîáîëüøîìó ñ÷åòó íåâàæíî, îñòàâëÿåì ëè ìû óæå çàìåíåííûå èëè íåò
            // òàê ÷òî ìîæíî çàìåíèòü CAS íà EXCH
            ////////////  H-bonds addition
            if (bnd->hatom != md->types[id2])  // another atom is not hydrogen (this is hydrogen)
            ////////////  end H-bonds addition
            ...
            //! (?) à ìîæåò áûòü àòîìèê è íå íóæåí
            //! îáúåäèíèòü ñ ïðåäûäóùèì áëîêîì, òîãäà ìîæíî èçáàâèòñÿ îò äâóõ if
        }
...
// recalculate number of atom types (ôóíêöèÿ ñäåëàíà íà òîò ñëó÷àé, åñëè íå íóæíî îáíîâëÿòü óãëû, à òèïû ÷àñòèö âñå ðàâíî ìåíÿþòñÿ)
{
    int id0 = blockIdx.x * atPerBlock + threadIdx.x * atPerThread;
    int N = min(id0 + atPerThread, md->nAt);
