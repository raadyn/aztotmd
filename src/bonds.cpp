...
// read bonds from file
{
   ...
   if (f == NULL)
   {
       ...
       //! âðåìåííî! ýòà øòóêà è çàäàåò ìàêñèìàëüíîå êîëè÷åñòâî ñâÿçè è àëëîöèðóåò ìåñòî. Íàäî ÷òîáû ìàêñèìàëüíîå ÷èñëî ñâÿçåé çàãðóæàëîñü èç äèðåêòèâû
...
//   (id) - index of the bond in bdata array
{
   ...
   // exp-6-8-12 potential:
     ...
     else if (strcmp(key, "mut") == 0)    // mutable bond
     {
         ...
         //! ýòî íóæíî äåëàòü, ïîñëå òîãî, êàê çàãðóçèì âñå ñâÿçè
     }
     else
     {
...
/* NOW CORRECT BOND TYPES VERIFIED NOT HERE
   // ñðàçó ðàçâåðíåì ñâÿçü êàê íàäî è ïðîâåðèì, ïðàâèëüíî ëè ñâÿçàíà
   if (field->bdata[type].spec1br == atm->types[at1])
      ...
      {
        //! òóò åù¸ ïðîâåðêó ïðàâèëüíûé ëè ïåðâûé àòîì
...
*/
   ...
   //!!!ïðîâåðêà âñå ëè ñâÿçè íîðìàëüíûå
   /*
   int flag = 0;
   int obond = -1;
   ...
   */
   ...
   if (i < field->mxBonds)
   {
      ...
      //! ñðàçó ïîìåíÿåì òèïû, ÷òîáû íå áûëî ïðîòèâîðå÷èé
      atm->types[at1] = t1aft;
      atm->types[at2] = t2aft;
...
void destroy_bond(int bnd, Atoms *atm, Field *field, Bond *bond)
{
   ...
   //! òåïåðü ýòî â íà÷àëå ïðîöåäóðû íà âñÿêèé ñëó÷àé
   atm->types[a1] = t1aft;
   atm->types[a2] = t2aft;

/*
   //!Ïîèñê äóáëèêàòíûõ ñâÿçåé
   for (j = 0; j < field->nBonds; j++)
     for (k = j + 1; k < field->nBonds - 1; k++)
        if (field->at1[j] > -1)
          if (((field->at1[j] == field->at1[k])&&(field->at2[j] == field->at2[k])) || ((field->at1[j] == field->at2[k])&&(field->at2[j] == field->at1[k])))
...
*/
   ...
   //! ïîñòàâèë ýòî â íà÷àëå ïðîöåäóðû äëÿ èçáåæàíèÿ ïåðåçàïèñè
   //atm->types[a1] = t1aft;
   //atm->types[a2] = t2aft;
