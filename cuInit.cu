...
// read cuda settings from "cuda.txt"
{
    ...
    //! добавить проверку, что св€занные траектории нужны
...
cudaMD* init_cudaMD(Atoms* atm, Field* fld, Sim* sim, TStat* tstat, Box* bx, Elec* elec, hostManagMD* man, cudaMD *h_md)
{
    ...
    //!  загрузить все ћѕ поровну, 1блок = 1ћѕ, опустим ситуацию, когда число ћѕ меньше кол-ва атомов, ведь дл€ моделировани€ нам нужны тыс€чи атомов
    man->atPerBlock = ceil((double)atm->nAt / man->nMultProc); // число атомов на ћѕ
...
// copy md results from device to host (hmd - host exemplar of cudaMD)
{
    ...
    //!!!!!!!!!
    // у нас —ќ–“»–ќ¬ ј! следовательно массивы чередуютс€ местами сортированный и нет! т.е. с хоста мы можем ссылатьс€ не на тот массив девайса
    // ! обновл€ем структуру
...
// hmd - host exemplar of cudaMD, dmd - on device
{
    ...
    //! внимение, везде, где используетс€ nCell и nAt в будущем надо перезагрузить эти пол€
