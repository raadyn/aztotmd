...
// read field parameters from field.txt (and reset counters)
{
  ...
  if (n = find_number(f, " linkage %d"))
  {
      if (!field->nBdata)
          ...
          {
              sim->use_bnd = 2;     // åñëè âñå õîðîøî, ïåðåâîäèì ôëàã use_bnd â çíà÷åíèå "èçìåíÿþùèåñÿ ñâÿçè"
              sim->use_linkage = 1;
          }
  }
...
// set up starting MD parameters
{
      ...
      else
      {
          ...
          if (!field->nBonds)
          {
              if (sim->use_linkage)
                  ...
                  sim->use_bnd = 0;   // åñëè ñâÿçåé ñðàçó íåò è íåò âîçìîæíîñòè èõ îáðàçîâûâàòüò, ñêèäâûâàåì ôëàã íà 0
          }
      }
  }
  else
  ...
  {
     sim->tTemp = tstat->Temp;  //! âîîáùåì-òî ýòî íåíóæíûé ïàðàìåòð, â sim - ôàêòè÷åñêàÿ òåìïåðàòóðà, â òåðìîñòàòå - öåëåâàÿ
  }
  else
     res = 0;
  ...
  if (sim->eJump != 0)
  {
        ...
        sim->rElec *= r_scale;     
        //sim->r2Elec *= r_scale;     //! óáðàòü èç sim ýòî ïîëå //! îïå÷àòêà? rElec instead of r2Elec? corrected above
        //sim->double_pars[dpMaxCutJump] = sim->r2Elec;
        sim->r2Elec = sim->rElec * sim->rElec;

        //! åñëè åñòü ñâÿçè, òî îíè ìîãóò ðàçðóøàòüñÿ èëè èçìåíÿòüñÿ â õîäå ýë ïåðåíîñà
        if (sim->use_bnd == 1)
            sim->use_bnd = 2;
  }
...
//! âûíåñòè ýòó ôóíêöèþ â ìîäóëü îòäåëüíûé äëÿ ñåðèéíîé âåðñèè, ÷òîáû ïîäêëþ÷àòü ïî íåîáõîäèìîñòè òîëüêî å¸
{
    int res = 1;    // function result
    //! çäåñü æå èíèöèàëèçàöèè rdf, ìàññèâîâ äëÿ Ýâàëüäà è ò.ä.
