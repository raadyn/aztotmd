...
// universal variant of pair interaction
{
    ...
    float3 delta;
    float r2 = delta_and_r2_shift(xyzA, xyzB, delta, shift);    //* èëè áåç øèôòà, åñëè â îäíîé ÿ÷åéêå

    if (r2 <= md->r2Max)    //* åñëè â îäíîé ÿ÷åéêå, è ðàçáèåíèå òèïà sqrt(3) - ìîæíî îïóñòèòü
    {
        float f = 0.f;
        float r = 0.f;      //* åñëè åñòü Êóëîí

        // electrostatic contribution
        if (md->use_coul)
            f += md->funcCoul(r2, r, md->chProd[typeA][typeB], md, engCoul);    //* åñëè åñòü Êóëîí

        // van der Waals contribution
        cudaVdW* vdw = md->vdws[typeA][typeB];
        if (vdw != NULL)
            if (r2 <= vdw->r2cut)   //* åñëè â îäíîé ÿ÷åéêå, è ðàçáèåíèå òèïà sqrt(3) - ìîæíî îïóñòèòü
            {
                if (!vdw->use_radii)
                {
                    if (md->use_coul)
                        f += vdw->feng_r(r2, r, vdw, engVdw); //* ëèáî âåðñèÿ òîëüêî ñ ñèëàìè, áåç ýíåðãèé, åñëè noCoul, òî åù¸ è áåç r - õîòÿ âñå ýòî íå äà¸ò íèêàêîé îïòèìèçàöèè
                    else
                        f += vdw->feng(r2, vdw, engVdw); //* ëèáî âåðñèÿ òîëüêî ñ ñèëàìè, áåç ýíåðãèé, åñëè noCoul, òî åù¸ è áåç r - õîòÿ âñå ýòî íå äà¸ò íèêàêîé îïòèìèçàöèè
                }
...
__device__ void global_pair(int i, int j, int iStep, cudaMD* md, float &engCoul, float &engVdw)
{
    ...
    if (r2 <= md->r2Elec)    //! cuttoff â ðÿäå ñëó÷àåâ ìîæíî îïóñòèòü ýòó ïðîâåðêó
    {
        float r = 0.f;
        float f = 0.f;
        int ti = md->types[i];
        ...
        // electrostatic contribution
        f += md->funcCoul(r2, r, md->chProd[ti][tj], md, engCoul);    //! åñëè ÷àñòèöû çàðÿæåíû

        // van der Waals contribution
        cudaVdW* vdw = md->vdws[ti][tj];
        if (vdw != NULL)
            if (r2 <= vdw->r2cut)   //! âíóòðè ÿ÷åéêè ýòó ïðîâåðêó ìîæíî îïóñòèòü
            {
                f += vdw->feng_r(r2, r, vdw, engVdw); //! ëèáî âåðñèÿ òîëüêî ñ ñèëàìè, áåç ýíåðãèé
#ifdef DEBUG_MODE
                atomicAdd(&(md->nVdWcall), 1);
#endif
            }
...
//! èä¸ì ÷óòü äàëüøå, åñëè ïîòîêîâ ñèëüíî ìíîãî ýòî ïî ïðåæíåìó áóäåò íåýôôåêòèâíî, íóæíî ñäåëàòü, ÷òîáû îäíà ïàðà îáðàáàòûâàëàñü íåñêîëüêèìè íèòÿìè
//! ó íàñ âñå âðåìÿ èä¸ò ïîâûøåíèå èåðàðõèè: ñíà÷àëà íèòü íà ÿ÷åéêó ñ ñîñåäÿìè, çàòåì íèòü íà ïàðó ÿ÷ååê, òåïåðü íåñêîëüêî íèòåé íà ïàðó ÿ÷ååê
//! äëÿ ýòîé ðåàëèçàöèè ñïèñîê àòîìîâ âíóòðè ÿ÷åéêè äîëæåí áûòü äâóìåðíûì (ýòî óâåëè÷èâàåò ðàçìåð, íî äîëæíî è óâåëè÷èòü ñêîðîñòü îáðàùåíèÿ)
    //! ÍÓÆÍÎ ÐÀÇÄÅËßÒÜ ÍÎÌÅÐ ß×ÅÉÊÈ ÔÀÊÒÈ×ÅÑÊÈÉ È ÍÎÌÅÐ ß×ÅÉÊÈ â ìàññèâå ids0, êîòîðûé äåëèò øàðåä ìàññèâû íà êóñêè ñîãëàñíî âíóòðåííåìó íîìåðó ÿ÷åéêè
    int xThreadPerPair = blockDim.x / pairStep;  // ÷èñëî thread â èçìåðåíèè x íà ïàðó (ñîâïàäàåò ñ ÷èñëîì ïîòîêîâ íà ÿ÷åéêó)
    ...
    //! íóæíî ïðîáåæàòüñÿ ïî âñåì ÿ÷åéêàì âñåõ ïàð, ÷òîáû çàïîìíèòü êîë-âà àòîìîâ â êàæäîé è íà÷àëüíûå èíäåêñû
    if (threadIdx.x == 0)
        if (threadIdx.y == 0)
        {
            engVdW = 0.0;
            ...
            /*
            //ïðîâåðèì êà, âñå ëè àòîìû â öåëë ëèñòå
            totAtm = 0;
            int mxAt = 0;
            int at = 0;
            if (blockIdx.x == 0)
    ...
    // íå çàáûâàåì, ÷òî ùàñ êîë-âî thread - äâóìåðíî, ïîýòîìó èíäåêñû ïîòîêîâ: [0..31][0..1]. y = 0 îáðàáàòûâàåò 1óþ ÿ÷åéêó â ïàðå, y=1 - âòîðóþ
    // thread index (the same for threadIdx.y = 0 and 1) in one cell/pair
    int iThread = threadIdx.x % xThreadPerPair;

    //! ñïåðâà ïîòîêè êîïèðóþò äàííûå â shared_memory

    step = ceil((double)nAts[intCell] / (double)xThreadPerPair);    // êîëè÷åñòâî àòîìîâ, îáðàáàòûâàåìûõ äàííûì ïîòîêîì
    sh = id0s[intCell];    // èíäåêñ â shared memory, â êîòîðûé íàäî íà÷èíàòü êîïèðîâàòü 
    id0 = iThread * step;
...
*/
    ...
    //! òóò ïîëó÷àåòñÿ â 2 ðàçà áîëüøå ïîòîêîâ, ïîñêîëêüó íà ïàðó
    //! âûáèðàåì ÿ÷åéêó ãäå áîëüøå àòîìîâ è îò íå¸¸ ïëÿøåì
    //! à íåò, ïåðåìåííàÿ cellShift ÷óâñòâèòåëüíà ê ïåðåñòàíîâêå i-j, ïîýòîìó ñîõðàíÿåì ñòàíäàòûé ïîðÿäîê

    intCell = intCell - threadIdx.y;    // ïîëó÷àåì âíóòð èíäåêñ ïåðâîé ÿ÷åéêè â ïàðå


    step = ceilf((double)nAts[intCell]/*nAt*/ / (double)(xThreadPerPair * 2));

    iThread = iThread * 2 + threadIdx.y;    // òåïåðü óæå ó íàñ â äâà ðàçà áîëüøå ïîòîêîâ
    ...
    {
        //md->frs[ids[i]] = frs[i];
        //! ïðèéäåòñÿ äåëàòü òàê, ïîñêîëüêó ñèëû äîáàâëÿþòñÿ óæå â apply_bonds ïåðåä ýòèì
        //! íåò, íå îáÿçàòåëüíî àòîìèê, ãëàâíîå +=
...
// without shared memory, so one block - one cell pair
{
    ...
    int step = ceil((double)na1 / blockDim.x);    // êîëè÷åñòâî àòîìîâ, îáðàáàòûâàåìûõ äàííûì ïîòîêîì
...
//! ïåðåíåñòè îïðåäåëåíèå nB1, nB2 è dim â init_cuda
{
#ifdef USE_CELL2
    //cell_list2a << < man->nPair1Block, man->nSingProc, man->memPerPairBlock >> > (devMD);
    cell_list_inSameCell << <1332, 4 >> > (devMD); // 4|16 - 123  8/16 - 124 ñ
#endif
#ifdef USE_CELL3
...
// äîïóñòèì íàäî ðàçäåëèòü ðàâíîìåðíî N ïîòîêîâ íà M ÷åãî-òî òàì, à íà öåëî îíè íå äåëÿòñÿ, ýòà ôóíêöèÿ êàê ðàç è âîçâðàùàåò:
//  íîìåð ïîòîêà âíóòðè ÷åãî-òî òàì (ÿ÷åéêè íàïðèìåð), ÷èñëî ïîòîêîâ â ýòîé ÿ÷åéêå nids è íîìåð ÿ÷åéêè divider_id
{
    if (divider == 0)
        printf("div by zero!!!\n");
...
//! ýòà ôóíêöèÿ îáðàáàòûâàåò ïàðû 0-1 2-3 4-5 ò.å. áåç ïåðåñå÷åíèÿ ïî àòîìàì. + âû÷èñëÿåò âçàèìîäåéñòâèÿ â ñàìèõ ÿ÷åéêàõ, à ïîòîì åù¸ è ïåðåñåêàþùèåñÿ (åëñè íåñêîëüêî ïàð)
// êàæäûé áëîê ñ÷èòûâàåò íåñêîëüêî ïàð, âñå áëîêè ñ÷èòûâàþò ïîëíîñòüþ âñå ÿ÷åéêè
__global__ void cell_list4a(int cellPerBlock, cudaMD* md)
// pairStep - step by pair numbers
//! îãðàíè÷åíèå: ôóíêöèÿ äîëæíà âûçûâàòüñÿ òàê, ÷òîáû ÷èñëî ïîòîêîâ íàöåëî äåëèëîñü íà ÷èñëî ÿ÷ååê
{
    ...
    //! èìååò ëè ñìûñë ýòè âåùè òîæå âûíåñòè â shared?
    ...
    //! ÷èñëî ïîòîêîâ äîëæíî áûòü áîëüøå ÷èñëà ÿ÷ååê
    ...
    float3 zero_shift = make_float3(0.f, 0.f, 0.f);
    int iCell;
    int iThread = get_internal_id(threadIdx.x, blockDim.x, nCell, iCell, step); // ñðàçó ïîëó÷èì è íîìåð ïîòîêà âíóòðè ÿ÷åéêè è íîìåð ÿ÷åéêè è ÷èñëî ïîòîêîâ íà ÿ÷åéêó (step)
    ...
    // äîïóùåíèå: ïîòîêîâ âñå ðàâíî >= ïàð ÿ÷ååê. Äëÿ 8 ÿ÷ååê ìàêñèìóì 4*7=28 ïàð, ïîòîêîâ äîëæíî áûòü áîëüøå (äåôîëòíî 64)
    ...
    {
        iThread = get_internal_id(threadIdx.x, blockDim.x, md->nFirstPairs[blockIdx.x], iPair, step); // ñðàçó ïîëó÷èì è íîìåð ïîòîêà âíóòðè ïðàû è íîìåð ïàðû è ÷èñëî ïîòîêîâ íà ïàðó (step)
        // à òåïåðü ó íàñ åñòü íåñêîëüêî ïîòîêîâ íà ïàðó. Êàæäûé ïîòîê ïóñòü âîçüì¸ò äèàïàçîí àòîìîâ ïåðâûîé ÿ÷åéêè ïàðû è âñå àòîìû âòîðîé ÿ÷åéêè:
...
//! áëîêè ïîëó÷àþò íà âõîä: èíäåêñ ïåðâîé ÿ÷åéêè, èíäåêñ âòîðîé ÿ÷åéêè, êîë-âî ÿ÷ååê ïîñëå íå¸ 
__global__ void cell_list4b(cudaMD* md)
{
    ...
    //INTERACTION WITH DIFFERENT CELLS
    // ðàçäåëÿåì ÷èñëî ïîòîêîâ ïîðîâíó ïî àòîìàì âòîðîé è ïîñëåäóþùåé ÿ÷åéêè, êàæäûé ïîòîê îáðàáàòûâàåò âçàèìîäåéñòâèå "ñâîåãî" äèàïàçîíà ñ àòîìàìè ïåðâîé ÿ÷åéêè
...
// íî íå ñîâñåì òàê, âñå ðàâíî ðàçäåëÿåìàÿ ïàìÿòü ïîíàäîáèàëñü
{
    int i, j, k;
    //int sh1, sh2, step;
...
*/
    ...
    */
    ...
    /*
        extern __shared__ int shMem[];      // dynamically allocated shared memory
        int* nAts = shMem;                  // number of atoms in a given cell
        int* fAts = (int*)&nAts[nCell + 1];     // first index of atom for a given cell (âî âíóòðåííèõ èíäåêñàõ)
...
// threadIdx.x is used
// åñëè ó íàñ ïîòîêè ðàçäåëåíû íåðàâíîìåðíî ïî ìàññèâó, òî äàííàÿ ôóíêöèÿ âû÷èñëÿåò âíóòðåííèé èä ïîòîêà, êîë-âî ïîòîêîâ íà ÿ÷åéêó ìàññèâà, è êîë-âî ïîòîêîâ â ÿ÷åéêå
{
    int i;
    for (i = n - 1; i > 0; i--)
...
// the same as 4a, but with smart distribution of threads
{
    ...
    if (nFillCell)
    {
        ...
        //if (cur_nat > 1)  òåïåðü ýòà ïðîâåðêà íå íóæíà, ïîòîìó ÷òî ìû àâòîìàòè÷åñêè ïîïàäàåì â íåïóñòóþ ÿ÷åéêó (õîòÿ ìîæåò áûòü âñå íåïóñòûå?)
        //{
        sh1 = fAts[iCell];
        i = 0;
        j = iThread + 1;     // first pair is 0-1
...
__device__ void interaction_by_ind(int i, int j, cudaMD* md, float& engVdw, float& engCoul)
{
    ...
    //if (r2 <= sim->r2Max)    //! cuttoff â ðÿäå ñëó÷àåâ ìîæíî îïóñòèòü ýòó ïðîâåðêó
    //{
    r = 0.0;
    f = 0.0;
    ...
    // electrostatic contribution
#ifdef USE_EWALD
    f += real_ewald(r2, r, md->chProd[typeA][typeB], md, engCoul);    //! åñëè ÷àñòèöû çàðÿæåíû
#else
#ifdef TX_CHARGE
    f += direct_coul(r2, r, tex2D(qProd, typeA, typeB), engCoul);    //! åñëè ÷àñòèöû çàðÿæåíû
#else
    f += direct_coul(r2, r, md->chProd[typeA][typeB], md, engCoul);    //! åñëè ÷àñòèöû çàðÿæåíû
#endif
#endif

    // van der Waals contribution
    //! èíîãäà ìîæíî íå ðàññìàòðèâàòü ýòîò áëîê, åñëè vdw çàâåäîìî âíå äåéñòâèÿ
...
__device__ void interaction_by_ind_wShift(int i, int j, int shift, int type, cudaMD* md, float& engVdw, float& engCoul)
{
    ...
    if (r2 <= md->r2Max)    //! cuttoff â ðÿäå ñëó÷àåâ ìîæíî îïóñòèòü ýòó ïðîâåðêó
    {
        r = 0.0;
        f = 0.0;
        int typeA = md->types[i];
...
__global__ void oneAtom_celllist(int atPerBlock, int atPerThread, int iStep, cudaMD* md)
{
    ...
    {
        //iCell = md->cellIndexes[iat];     //! òàê äåëàòü íåëüçÿ, ïîòîìó ÷òî ýòîò ìàññèâ íå áûë îòñîðòèðîâàí, â îòëè÷èå îò îñòàëüíûõ
        //! ïîýòîìó âû÷èñëèì ÿ÷åéêó çàíîâî (õîòÿ ìîæíî áûëî îòñîðòèðîâàòü)
...
// íå ìåíüøå 4õ ïîòîêîâ!
{
    ...
    // òóïîé ñïîñîá îáõîäà, êàæäûé ïîòîê ïîëó÷àåò ñâîé i è ïåðåáèðàåò j
    // åñòü íåäîñòàòîê - ïîòîêè çàãðóæåíû íå ðàâíîìåðíî
