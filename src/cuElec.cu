...
extern __constant__ const float d_sqrtpi;  //sqrt(PI); - íå ïîääåðæèâàåòñÿ äèíàìè÷åñêàÿ èíèöèàëèçàöèÿ
...
{
    float kqq = chprd * d_Fcoul_scale;  //! íóæíî åù¸ äîáàâèòü 1/epsilon
    r = sqrt(r2);
...
// create textures for evaluation of real part of Ewald summ
{
    ...
    //! ê ñîæàëåíèþ òàê è íå ïîëó÷èëîñü ïðèðó÷èòü cudaBindTexture, ïðàâäà íàïèñàíî, ÷òî îíî è íå ïîääåðæèâàåò ôèëüòðàöèþ
    //! ïðèéäåòñÿ èñïîëüçîâàòü bindTextureArray
...
//! òóò íàäî åù¸ ââåñòè ýïñèëîí â çàêîí êóëîíà
{
    ...
    eng += kqq * erfcar / r;       //! save energy (íå âñåãäà îíà íóæíà)
...
//! òóò íàäî åù¸ ââåñòè ýïñèëîí â çàêîí êóëîíà
{
    ...
    eng += kqq * (erfcar * ir - md->elC1 + md->elC2 * (r - md->rElec));    //! save energy (íå âñåãäà îíà íóæíà)
...
// the first part : summ (qiexp(kr)) evaluation
{
    ...
    for (i = id0; i < N; i++)
    {
        ...
        // â îðèãèíàëå ýòè äâå ïåðåìåííûå îïðåäåëåíû íà ýòàïå èíèöèàëèçàöèè
        ...
        for (l = 0; l < nkx; l++)
        {
            ...
            for (m = mmin; m < nky; m++)
            {
                ...
                if (m >= 0)
                {
                        lm.x = el[0].x * em[m].x - el[0].y * em[m].y;       // [0] - ïîòîìó ÷òî ìû ïåðåçàïèñûâàåì ýëåìåíò [l] â [0]
