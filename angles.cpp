...
void read_anglelist(Atoms *atm, Field *field, Sim *sim)
{
   ...
   if (f == NULL)
   {
       ...
       //! временно! эта штука и задает максимальное количество углов и аллоцирует место. Надо чтобы максимальное число углов загружалось из директивы
   ...
   if (n)
       ...
       if (sim->use_angl == 2) // если не задано углов, но мы знаем, что они могут образовываться - выделяем память
