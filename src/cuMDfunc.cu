...
//! only for rectangular geometry
{
    ...
    if (dx > md->halfLeng.x) // âòîðîé àòîì â îòðèöàòåëüíîì îòîáðàæåíèè
    {
        px = -1;
    }
    else
        if (dx < -md->halfLeng.x) //  âòîðîé àòîì â ïîëîæèòåëüíîì îòîáðàæåíèè
        {
            px = 1;
        }
        else
...
// save atom index with coordinates xyz in the cell list
{
    ...
    //! èëè ìîæåò íå âû÷èñëÿòü ýòî, à õðàíèòü èíäåêñ ÿ÷åéêè â òðåõìåðíîì ìàññèâå â texture_memory
    //!    è îáðàùàòüñÿ ïî ïðèâåäíåííûì êîîðäèíàòì [x / md->cSize.x][y / md->cSize.y]
    //!    åñòü ëè â ýòîì ñìûñë?
...
// the first stage of velocity verlet algorithm with neighbors saving
{
    ...
    if (threadIdx.x == 0)   // 0th thread of 0th block, reset some energies
    {
        ...
        if (blockIdx.x == 0)
        {
            //     md->engElecField = 0;  //! óáðàòü â reset
#ifdef TX_CHARGE
            /*
            float p1, p2, p3, p4;
            p1 = tex2D(qProd, 0, 0);
    ...
    for (i = id0; i < N; i++)
    {
        ...
        //! ñáðîøó çäåñü, õîòÿ ïðè èñïîëüçîâàíèè cell_list ýòî íå îáÿçàòåëüíî, ýòî íóæíî åñëè ìû èñïîëüçóåì all_pair
        //! òåïåðü îáÿçàòåëüíî, ïîñêîëüêó apply_bonds âûïîëíÿåòñÿ äî cell_list
        //! à åù¸ îáÿçàòåëüíî, ïîñêîëüêó èñïîëüçóåòñÿ âíåøíåå ýë. ïîëå
